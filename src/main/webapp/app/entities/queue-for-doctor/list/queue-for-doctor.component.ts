...
@Component({
  ...
  templateUrl: './queue-for-doctor.component.html',
})
export class QueueForDoctorComponent implements OnInit {
  queueForDoctors?: IQueueForDoctor[];
  isLoading = false;
  ...
  trackId = (_index: number, item: IQueueForDoctor): number => this.queueForDoctorService.getQueueForDoctorIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.queueForDoctors = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IQueueForDoctor[] | null): IQueueForDoctor[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.queueForDoctorService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
