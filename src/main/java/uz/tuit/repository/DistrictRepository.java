...
public interface DistrictRepository extends JpaRepository<District, Long>, JpaSpecificationExecutor<District> {
    ...
    @Query(
        value = "select distinct district from District district left join fetch district.region",
    ...
    @Query("select distinct district from District district left join fetch district.region")
    List<District> findAllWithToOneRelationships();

    @Query("select district from District district left join fetch district.region where district.id =:id")
