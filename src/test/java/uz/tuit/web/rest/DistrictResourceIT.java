...
class DistrictResourceIT {
    ...
    @Test
    @Transactional
    void getAllDistrictsByRegionIsEqualToSomething() throws Exception {
        Region region;
        if (TestUtil.findAll(em, Region.class).isEmpty()) {
            districtRepository.saveAndFlush(district);
            region = RegionResourceIT.createEntity(em);
        ...
        // Get all the districtList where region equals to regionId
        defaultDistrictShouldBeFound("regionId.equals=" + regionId);

        // Get all the districtList where region equals to (regionId + 1)
        defaultDistrictShouldNotBeFound("regionId.equals=" + (regionId + 1));
    }
