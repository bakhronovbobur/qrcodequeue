...
class DepartmentResourceIT {
    ...
    @Test
    @Transactional
    void getAllDepartmentsByHospitalIsEqualToSomething() throws Exception {
        Hospital hospital;
        if (TestUtil.findAll(em, Hospital.class).isEmpty()) {
            departmentRepository.saveAndFlush(department);
            hospital = HospitalResourceIT.createEntity(em);
        ...
        // Get all the departmentList where hospital equals to hospitalId
        defaultDepartmentShouldBeFound("hospitalId.equals=" + hospitalId);

        // Get all the departmentList where hospital equals to (hospitalId + 1)
        defaultDepartmentShouldNotBeFound("hospitalId.equals=" + (hospitalId + 1));
    }

    @Test
    @Transactional
    void getAllDepartmentsByDoctorsIsEqualToSomething() throws Exception {
        Doctor doctors;
        if (TestUtil.findAll(em, Doctor.class).isEmpty()) {
            departmentRepository.saveAndFlush(department);
            doctors = DoctorResourceIT.createEntity(em);
        } else {
            doctors = TestUtil.findAll(em, Doctor.class).get(0);
        }
        em.persist(doctors);
        em.flush();
        department.addDoctors(doctors);
        departmentRepository.saveAndFlush(department);
        Long doctorsId = doctors.getId();

        // Get all the departmentList where doctors equals to doctorsId
        defaultDepartmentShouldBeFound("doctorsId.equals=" + doctorsId);

        // Get all the departmentList where doctors equals to (doctorsId + 1)
        defaultDepartmentShouldNotBeFound("doctorsId.equals=" + (doctorsId + 1));
    }
